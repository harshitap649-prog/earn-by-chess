...
function evaluatePawnStructure(game: Chess, color: string): number {
  const board = game.board()
  let score = 0
  const pawns: number[] = []
  ...
  // Passed pawns bonus
  pawns.forEach(file => {
    const rank = board.findIndex(row => row[file]?.type === 'p' && row[file]?.color === color)
    if (rank !== -1) {
      const isWhite = color === 'w'
      let isPassed = true
...
function evaluateKingSafety(game: Chess, color: string): number {
  const board = game.board()
  let kingRank = -1
  let kingFile = -1
...
function quiescence(
  ...
  // Limit quiescence depth - increased for stronger tactical play
  ...
  // Limit number of tactical moves - increased for better tactical play
  const MAX_TACTICAL_MOVES = 10
...
// Professional chess AI with deep search
export function getComputerMove(game: Chess): { from: string; to: string; promotion?: string } | null {
  try {
    const moves = game.moves({ verbose: true })
    ...
    // Calculate optimal depth - PRO LEVEL for free matches (computer should always win)
    // Use deeper depth for stronger play - prioritize strength over speed
    let depth = 4 // Base depth for PRO level play
    
    // Increase depth in simpler positions for even stronger play
    if (isEndgamePos && numMoves < 15) {
      depth = 5 // Very deep in endgame
    }
    
    if (numMoves < 12) {
      depth = 5 // Very deep when few moves available
    }
    
    // Still strong even in complex positions
    if (numMoves > 30) {
      depth = 3 // Still strong in complex positions
    }
    ...
    for (const move of moves) {
      ...
      moveScores.push({ move, score: priority })
    }
    
    // Sort moves - best first
    moveScores.sort((a, b) => isMaximizing ? (b.score - a.score) : (a.score - b.score))
    
    // Evaluate more moves for stronger play - PRO level
    const MAX_MOVES_TO_EVALUATE = numMoves > 25 ? 20 : (numMoves > 20 ? 18 : numMoves)
